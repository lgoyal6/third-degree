...
### Question ranking: learned, measured, and never trusted blindly

Question order inside a grill session is now a measured recommendation problem instead of a heuristic. The ladder is untouchable - fundamentals still come before seams - but *within* each layer a small learned model (`src/lib/ranker/`) can reorder questions by predicted miss probability, so the question most likely to sting gets one of the session's limited slots first.

- **The model is small on purpose.** Logistic regression over nine named features (similarity to the concepts the review queue marked due, per-kind miss rate and support, prerequisite readiness, repository coverage, prior completion, recency, ladder layer, grading tier), trained by full-batch gradient descent with zero init and fixed epochs, so training is bit-for-bit reproducible. The committed artifact ([src/lib/ranker/model.v1.json](./src/lib/ranker/model.v1.json)) is plain JSON; every weight has a name you can read.
- **The dataset is versioned and real.** `npm run rank:dataset` builds one row per graded attempt from the product's stored sessions - no synthetic outcomes, no answer text, session ids hashed, private repos excluded. The JSONL stays local (gitignored); [data/ranking/manifest.json](./data/ranking/manifest.json) pins the version, sha256, counts, and the frozen split spec, and the eval refuses data that has drifted from the manifest.
- **Cold start is deterministic.** A new user ranks on global difficulty priors frozen into the artifact, same input giving the same order every time; a new repository sits at neutral history values; a question kind the model never trained on (or any out-of-distribution feature vector) keeps its baseline slot while the rest reorder around it.
- **The old behavior is the fallback.** No model, an invalid artifact, out-of-distribution input, or `RANKER_OFF=1` all return the assembled baseline order untouched. End-to-end tests (`src/lib/grill/generate.e2e.test.ts`) drive the real indexer over a fixture repo and pin every fallback and cold-start path.
- **Rollout is a holdout, not a launch.** Each session is deterministically assigned baseline or learned 50/50 by a stable hash of the user unit (GitHub id, else a stable anonymous browser id, else per-session), the arm is stamped on the session with the unit hashed, and exposure events are logged. No online improvement is claimed anywhere: no assigned traffic exists yet.

`npm run rank:eval` is the one-command offline evaluation. It verifies the frozen dataset hash, runs a leakage detector that must also trip on a leak the harness plants every run, then prints NDCG@3 / MRR / Recall@3 / coverage with bootstrap confidence intervals for the current baseline order, a popularity baseline, a seeded random ranker, the learned model, and a shuffled-label negative control, plus calibration on P(miss) and repository-holdout slices. The honest readout on dataset v1 (89 attempts, mostly the developer's own QA traffic): **the learned ranker currently loses to every baseline**, which is exactly why it ships behind the holdout with the baseline as the fallback rather than as the new default. `npm run rank:train` refits the model from the frozen training split.
...
- ✅ **Learned question ranking, measured** (Sep 2026): versioned real-event dataset, nine-feature logistic re-ranker behind a deterministic 50/50 holdout, one-command offline eval with baselines and negative controls (`npm run rank:eval`), baseline order as the safe fallback. Offline result so far is honestly negative; no online claim until real assigned traffic exists
